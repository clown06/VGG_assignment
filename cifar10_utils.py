...
def dense_to_one_hot(labels_dense, num_classes):
  """
  Convert class labels from scalars to one-hot vectors.标量转化为onehot
  Args:
...
class DataSet(object):
  ...
  """
  Function: Additional functions in the Dataset class, for the convenience of directly using DataLoader
  __len__:
  ...
  # Return samples from the dataset
  def __getitem__(self, index):
    # Ensure that the index is within the scope of the dataset
...
def read_data_sets(data_dir, one_hot = True, validation_size = 0):
  ...
  Uses or not uses one-hot encoding for the labels./是否使用one_hot编码
